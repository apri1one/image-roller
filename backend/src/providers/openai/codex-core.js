...
export class CodexApiService {
    ...
    async prepareRequestBody(model, requestBody, stream) {
        ...
        if (isImageModel) {
            ...
            // 默认走 thinking 模式（gpt-5.4 + image_generation tool 高 reasoning effort）
            // 对应 ChatGPT 网页生图的 Thinking 模式（vs Instant）
            // 客户端如需切回 instant 可在 request body 显式传 reasoning.effort = 'low'
            if (!cleanedBody.reasoning) cleanedBody.reasoning = {};
            if (!cleanedBody.reasoning.effort) cleanedBody.reasoning.effort = 'high';
            logger.info(`[Codex] Image model detected: ${upstreamModel} -> ${effectiveUpstreamModel} with image_generation tool${imageToolConfig.size ? `, size=${imageToolConfig.size}` : ''}, reasoning.effort=${cleanedBody.reasoning.effort}`);
        } else {
            // 为普通 Codex 模型增加默认工具
