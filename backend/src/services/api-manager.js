...
async function handleImageGenerationRequest(req, res, currentConfig, providerPoolManager, retryContext = null) {
    ...
    try {
        if (retryContext?.parsedBody) {
            ...
            // 透传 reasoning（让前端可切换 instant/thinking 模式）
            // 客户端传 reasoning: { effort: 'high' } 走 thinking，'low' / 'minimal' 走 instant
            // 不传时 codex-core.js 会给 image model 默认 'high'
            if (body.reasoning) virtualOpenAIRequest.reasoning = body.reasoning;
...
async function handleImageEditsRequest(req, res, currentConfig, providerPoolManager) {
    ...
    try {
        ...
        // 提取 mask（可选，用于 inpainting 局部编辑）
        const maskFile = files.mask;
        let maskUrl = null;
        if (maskFile && maskFile.buffer && maskFile.buffer.length > 0) {
            maskUrl = `data:${maskFile.mimetype || 'image/png'};base64,${maskFile.buffer.toString('base64')}`;
            logger.info(`[Image Edits] Mask provided: ${Math.round(maskFile.buffer.length / 1024)}KB`);
        }
        ...
        if (maskUrl) {
            contentArray.push(
                { type: 'text', text: 'The following image is an inpainting mask. White areas indicate regions to edit; black areas should be preserved unchanged.' },
